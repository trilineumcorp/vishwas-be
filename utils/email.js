...
const createTransporter = () => {
    ...
    if (!config.auth) {
        logger_1.logger.warn('No email credentials found. Email sending will be disabled.');
        logger_1.logger.warn('To enable email sending, set EMAIL_USER and EMAIL_PASSWORD in .env file');
        logger_1.logger.warn(`Current EMAIL_USER: ${process.env.EMAIL_USER || 'NOT SET'}`);
        logger_1.logger.warn(`Current EMAIL_PASSWORD: ${process.env.EMAIL_PASSWORD ? 'SET (hidden)' : 'NOT SET'}`);
        logger_1.logger.warn(`Current EMAIL_HOST: ${config.host}`);
        logger_1.logger.warn(`Current EMAIL_PORT: ${config.port}`);
        return null;
    }
    try {
        ...
        logger_1.logger.info(`Email transporter created successfully for ${config.host}:${config.port}`);
        return transporter;
    }
    catch (error) {
        logger_1.logger.error('Failed to create email transporter:', error);
        return null;
    }
};
const sendEmail = async (options) => {
    try {
        ...
        if (!transporter) {
            const config = getEmailConfig();
            logger_1.logger.warn('Email transporter not available. Email not sent.');
            logger_1.logger.warn(`Email config check - EMAIL_USER: ${config.auth ? 'SET' : 'NOT SET'}, EMAIL_PASSWORD: ${config.auth ? 'SET' : 'NOT SET'}`);
            logger_1.logger.warn(`Would have sent email to: ${options.to} with subject: ${options.subject}`);
            return false;
        }
        const config = getEmailConfig();
        const mailOptions = {
        ...
        logger_1.logger.info(`Attempting to send email to: ${options.to} from: ${config.from}`);
        const info = await transporter.sendMail(mailOptions);
        logger_1.logger.info(`Email sent successfully to ${options.to}. Message ID: ${info.messageId}`);
        logger_1.logger.info(`Email response: ${JSON.stringify(info.response)}`);
        return true;
    }
    catch (error) {
        ...
        if (error.code === 'EAUTH') {
            logger_1.logger.error('Authentication failed. Please check EMAIL_USER and EMAIL_PASSWORD in .env file');
        }
        else if (error.code === 'ECONNECTION') {
            logger_1.logger.error('Connection failed. Please check EMAIL_HOST and EMAIL_PORT in .env file');
        }
        else if (error.code === 'ETIMEDOUT') {
            logger_1.logger.error('Connection timeout. Please check your network and SMTP settings');
        }
        return false;
    }
};
